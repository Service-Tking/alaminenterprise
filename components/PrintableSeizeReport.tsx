...
const PrintableSeizeReport: React.FC<PrintableSeizeReportProps> = ({ data, onClose }) => {
  const renderCheckmark = (state: PaperState) => {
    if (state === true) return <span className="font-bold text-[10px]">✔</span>;
    if (state === 'cross') return <span className="font-bold text-[10px]">✖</span>;
    return null;
  };
      ...
      <div className="bg-white printable-a4 w-[210mm] h-[297mm] p-[10mm] text-black shadow-2xl print:shadow-none print:m-0 print:w-full font-serif text-[11px] leading-tight flex flex-col border overflow-hidden relative">
        ...
        <div className="border border-black mb-1 bg-white shrink-0">
          <div className="grid grid-cols-2 divide-x divide-black border-b border-black h-7 items-center">
             <div className="flex px-2 h-full items-center"><span className="w-32 font-bold uppercase text-[9px]">Cust ID No:</span> <span className="flex-1 font-black">{data.customerIdNo}</span></div>
             <div className="flex px-2 h-full items-center"><span className="w-32 font-bold uppercase text-[9px]">Reg No:</span> <span className="flex-1 font-black">{data.registrationNo}</span></div>
          </div>
          <div className="grid grid-cols-2 divide-x divide-black border-b border-black h-7 items-center">
             <div className="flex px-2 h-full items-center"><span className="w-32 font-bold uppercase text-[9px]">Cust Name:</span> <span className="flex-1 font-black uppercase">{data.customerName}</span></div>
             <div className="flex px-2 h-full items-center"><span className="w-32 font-bold uppercase text-[9px]">Chassis No:</span> <span className="flex-1 font-mono text-[10px] uppercase font-black">{data.chassisNo}</span></div>
          </div>
          <div className="flex px-2 border-b border-black h-7 items-center">
          ...
          <div className="grid grid-cols-2 divide-x divide-black h-7 items-center">
             <div className="flex px-2 h-full items-center"><span className="w-32 font-bold uppercase text-[9px]">Officer:</span> <span className="flex-1 uppercase font-black">{data.officerName}</span></div>
             <div className="flex px-2 h-full items-center"><span className="w-32 font-bold uppercase text-[9px]">Name of Depo:</span> <span className="flex-1 font-black">{data.nameOfDepo}</span></div>
          </div>
        </div>

        {/* Papers Section */}
        <div className="border border-black p-1.5 mb-1 bg-white text-[10px] shrink-0">
           <span className="font-bold underline mb-1 block uppercase text-[9px]">Papers Checklist:</span>
           <div className="grid grid-cols-4 gap-y-1 gap-x-2 px-2">
              {[
                ...
                <div key={idx} className="flex items-center gap-2 h-4">
                   ...
                   <span className="text-[10px] font-black uppercase whitespace-nowrap leading-none">{p.label}</span>
                </div>
              ))}
           </div>
        </div>

        {/* Inspection Grid - Highly Compressed for One-Page Fit */}
        ...
        <div className="border border-black mb-1.5 flex flex-col h-12 shrink-0 overflow-hidden">
        ...
        {/* Footer Area */}
        <div className="grid grid-cols-3 gap-6 px-1 mb-4 text-[11px] font-bold text-center uppercase shrink-0">
           <div className="flex flex-col items-center">
              <span className="mb-6 border-b border-gray-400 w-full pb-0.5">Assigner Signature</span>
              <div className="text-left w-full text-[10px] mt-2">
                 <p className="flex justify-between"><span>Name:</span> <span className="border-b border-black flex-1 ml-1 font-black">{data.assigner.name}</span></p>
              </div>
           </div>
           <div className="flex flex-col items-center">
              <span className="mb-6 border-b border-gray-400 w-full pb-0.5">Officer Signature</span>
              <div className="text-left w-full text-[10px] mt-2">
                 <p className="flex justify-between"><span>Name:</span> <span className="border-b border-black flex-1 ml-1 font-black">{data.officerName}</span></p>
              </div>
           </div>
           <div className="flex flex-col items-center">
              <span className="mb-6 border-b border-gray-400 w-full pb-0.5 uppercase">Authorized Incharge</span>
              <div className="text-left w-full text-[10px] mt-2 font-black leading-tight">
                 <p>{data.depoSignatory.name}</p>
                 <p className="text-gray-600 font-bold">{data.depoSignatory.mobile}</p>
              </div>
           </div>
        </div>
